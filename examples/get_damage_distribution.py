...
if __name__ == '__main__':
    ...
    # Extract DUT map
    fluence_map_DUT, bin_c_x_DUT, bin_c_y_DUT = fluence.extract_dut_map(fluence_map=fluence_map,
                                                                        map_bin_centers_x=bin_c_x,
                                                                        map_bin_centers_y=bin_c_y,
                                                                        irrad_data=data[irrad_server]['Irrad'])

    # Extract respective dut error map
    error_map_DUT, _, _ = fluence.extract_dut_map(fluence_map=error_map,
                                                  map_bin_centers_x=bin_c_x,
                                                  map_bin_centers_y=bin_c_y,
                                                  irrad_data=data[irrad_server]['Irrad'])
    ...
    tid_map_DUT = formulas.tid_per_scan(primary_fluence=fluence_map_DUT, stopping_power=server_config['daq']['stopping_power'])
    tid_error_map_DUT = formulas.tid_per_scan(primary_fluence=error_map_DUT, stopping_power=server_config['daq']['stopping_power'])
    ...
    print(f"NEQ fluence map on DUT [neq/cm²] (bins={neq_map_DUT.shape}, mean={neq_map_DUT.mean():.2E}):\n\n{neq_map_DUT}\n\n")
