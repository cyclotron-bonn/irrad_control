...
if __name__ == "__main__":
    ...
    neq_fluence_error = (
        (server_config["daq"]["kappa"]["nominal"] * damage_data["scan_primary_fluence_error"]) ** 2
        + (damage_data["scan_primary_fluence"] * server_config["daq"]["kappa"]["sigma"]) ** 2
    ) ** 0.5
    ...
    for i in range(len(neq_fluence)):
        print(
            f"{damage_data['scan'][i]}\t | \t {neq_fluence[i]:.2E}+-{neq_fluence_error[i]:.2E} | \t {damage_data['scan_tid'][i]:.1f}+-{damage_data['scan_tid_error'][i]:.1f}"
        )
