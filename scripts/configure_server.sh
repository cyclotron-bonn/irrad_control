...
if [ "$MINICONDA_PATH" == false ]; then
  ...
  echo "source ${MINICONDA_PATH}/bin/activate; irrad_server" > ${IRRAD_PATH}/scripts/start_server.sh
  ...
  if [ "$CURR_PY_VERSION" == "$PY_VERSION" ]; then
    ...
    if [ ! -d "$MINICONDA_PATH/envs" ]; then
      ...
      # Create server start script for server
      echo "source ${MINICONDA_PATH}/bin/activate; conda activate py${PY_VERSION}; irrad_server" > ${IRRAD_PATH}/scripts/start_server.sh
      ...
      if [ "$MATCH_ENV" == false ]; then
        ...
        # Create server start script for server
        echo "source ${MINICONDA_PATH}/bin/activate; conda activate ${MATCH_ENV}; irrad_server" > ${IRRAD_PATH}/scripts/start_server.sh
      fi
    fi
  fi
fi
