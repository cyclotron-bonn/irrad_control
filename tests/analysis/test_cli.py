...
class TestAnalysisCLI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.fixtures = {
            "calibration": os.path.join(cls.fixture_path, "test_calibration"),
    ...
    def _run_cli_analysis(self, analysis, infile, flags=None, cli_str=None):
        if cli_str is not None:
            ...
            # Run CLI and check return status
            success = os.system(
                "irrad_analyse -f {} --{} -o {} {}".format(
                    " ".join(infile), analysis, outfile, "" if flags is None else " ".join(flags)
                )
            )
    ...
    def test_multifile_analysis(self):
        cli_str = "irrad_analyse -f {} --scan -o {}".format(
            " ".join(self.fixtures["multipart"]),
            " ".join(
                [os.path.join(self.output_dir, x) for x in ["test_mutlifile_scan1.pdf", "test_mutlifile_scan2.pdf"]]
            ),
        )
