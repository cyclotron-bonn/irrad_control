...
class TestConverter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Load the data
        cls.data, cls.config = load_irrad_data(
            data_file=cls.test_base + ".h5", config_file=cls.test_base + ".yaml", subtract_raw_offset=False
        )
    ...
    def _check_output_data(self):
        # Open output file
        out_data, _ = load_irrad_data(
            data_file=self.config["session"]["outfile"] + ".h5",
            ...
            for dname in out_data[self.server][data].dtype.names:
                ...
                if data == "Raw":
                    np.testing.assert_array_equal(
                        out_data[self.server][data][dname], self.data[self.server][data][dname]
                    )
    ...
    def _send_scan_data(self, status, scan_idx=None):
        if status == "scan_init":
            meta = {
                "timestamp": float(self.data[self.server]["Irrad"]["timestamp"][0]),
                "name": "localhost",
                "type": "scan",
            }
            data = {
                ...
                "scan_origin": (
                    float(self.data[self.server]["Irrad"]["scan_origin_x"][0]),
                    float(self.data[self.server]["Irrad"]["scan_origin_y"][0]),
                ),
                "scan_area_start": (
                    float(self.data[self.server]["Irrad"]["scan_area_start_x"][0]),
                    float(self.data[self.server]["Irrad"]["scan_area_start_y"][0]),
                ),
                "scan_area_stop": (
                    float(self.data[self.server]["Irrad"]["scan_area_stop_x"][0]),
                    float(self.data[self.server]["Irrad"]["scan_area_stop_y"][0]),
                ),
                "dut_rect_start": (
                    float(self.data[self.server]["Irrad"]["dut_rect_start_x"][0]),
                    float(self.data[self.server]["Irrad"]["dut_rect_start_y"][0]),
                ),
                "dut_rect_stop": (
                    float(self.data[self.server]["Irrad"]["dut_rect_stop_x"][0]),
                    float(self.data[self.server]["Irrad"]["dut_rect_stop_y"][0]),
                ),
                "beam_fwhm": (
                    float(self.data[self.server]["Irrad"]["beam_fwhm_x"][0]),
                    float(self.data[self.server]["Irrad"]["beam_fwhm_y"][0]),
                ),
            }
            ...
            # Publish data
            meta = {
                "timestamp": float(self.data[self.server]["Scan"]["row_start_timestamp"][scan_idx]),
                "name": "localhost",
                "type": "scan",
            }
            data = {
            ...
            # Publish stop data
            meta = {
                "timestamp": float(self.data[self.server]["Scan"]["row_stop_timestamp"][scan_idx]),
                "name": "localhost",
                "type": "scan",
            }
            data = {
            ...
            # Put finished data
            meta = {
                "timestamp": float(self.data[self.server]["Result"]["timestamp"][0]),
                "name": "localhost",
                "type": "scan",
            }
            data = {"status": status}
        ...
        try:
            if not self._scan_started:
                ...
                if self.data[self.server]["Scan"]["scan"][self._scan_stop_idx] != -1:
                    if (
                        self.data[self.server]["Damage"]["scan"][self._scan_idx]
                        != self.data[self.server]["Scan"]["scan"][self._scan_stop_idx]
                    ):
                        self._send_scan_data(status="scan_complete", scan_idx=self._scan_idx)
                        self._scan_idx += 1
