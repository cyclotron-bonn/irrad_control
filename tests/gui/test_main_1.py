...
class TestMain(unittest.TestCase):
    ...
    def test_setup_tab_finding_servers(self):
        ...
        # Create test state
        self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets['8.8.8.8']['checkbox'].setChecked(True)

        # Check that all available IPs have been found
        self.assertTrue(all(i in self.irrad_window.setup_tab.session_setup.setup_widgets['network'].available_servers for i in ('8.8.8.8', '8.8.4.4')))

        # Check that not available IPs have not been found
        self.assertFalse('8.8.4.123' in self.irrad_window.setup_tab.session_setup.setup_widgets['network'].available_servers)

        # Check that default server has been selected
        self.assertTrue(self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets[network_config['server']['default']]['checkbox'].isChecked())

    def test_setup_tab_server_tab_creation(self):

        # Create test state
        for ip in self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets:
            self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets[ip]['checkbox'].setChecked('8.8.8.8' == ip)
        ...
        # Remove default tab
        self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets[network_config['server']['default']]['checkbox'].setChecked(False)

        # Check if we have 0 tab
        self.assertTrue(self.irrad_window.setup_tab.server_setup.tabs.count() == 0)

        for ip in self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets:
            self.irrad_window.setup_tab.session_setup.setup_widgets['selection'].widgets[ip]['checkbox'].setChecked(True)
