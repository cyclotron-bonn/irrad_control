...
class IrradServer(DAQProcess):
    ...
    def _init_devices(self):
        ...
        self.axis_tracker = BaseAxisTracker(context=self.context, address=self._internal_sub_addr, sender=self.server)
    ...
    def _setup_devices(self):
        ...
        if "IrradDAQBoard" in self.devices and self.setup["server"]["readout"]["device"] == RO_DEVICES.DAQBoard:
            ...
            self.devices["IrradDAQBoard"].set_ifs(
                group="sem", ifs=self.setup["server"]["readout"]["ro_group_scales"]["sem"]
            )
            self.devices["IrradDAQBoard"].set_ifs(
                group="ch12", ifs=self.setup["server"]["readout"]["ro_group_scales"]["ch12"]
            )
        ...
        if "ScanStage" in self.devices:
            ...
            self.devices["__scan__"] = DUTScan(scan_stage=self.devices["ScanStage"], irrad_events=self.irrad_events)

            # Connect to ZMQ
            self.devices["__scan__"].setup_zmq(
                ctx=self.context, skt=self.socket_type["data"], addr=self._internal_sub_addr, sender=self.server
            )

        if "RadiationMonitor" in self.devices:
            # Add custom methods for being able to pause/resume data sending
            self.devices["RadiationMonitor"]._send_data = lambda send: getattr(
                self.stop_flags["wait_rad_mon"], "set" if send else "clear"
            )()
    ...
    def handle_cmd(self, target, cmd, data=None):
        ...
        # Handle server commands
            ...
            if cmd == "start":
                ...
                reply_data = {
                    ms: {"positions": self.devices[ms].get_positions(), "props": self.devices[ms].get_physical_props()}
                    for ms in self._motorstages
                }
    ...
    def handle_event(self, event_data):
        ...
        try:
            ...
            logging.debug(
                f"Event {event_data['event']} on server {self.name} is {'' if event_data['active'] else 'in'}active"
            )
        except KeyError:
