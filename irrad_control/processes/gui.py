...
class IrradGUI(QtWidgets.QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.sub_splitter.setSizes([int(1.0 / 3.0 * self.width()), int(2.0 / 3.0 * self.width())])
    ...
    def _init_tabs(self):
        ...
        for name in self.tab_order:
            if name == "Setup":
                ...
                self.setup_tab.session_setup.setup_widgets["session"].widgets[
                    "logging_combo"
                ].currentTextChanged.connect(lambda lvl: self.log_widget.change_level(lvl))
    ...
    def _init_progress_dialog(self):
        ...
        pdiag_label = QtWidgets.QLabel(
            "Launching application:\n\n->Staring data converter...\n->Configuring {0} server(s)...\n->Starting {0} server(s)...".format(
                len(self.setup["server"])
            )
        )
    ...
    def _init_processes(self):
        ...
        for server in self.setup["server"]:
            ...
            server_config_workers[server] = QtWorker(
                func=self.proc_mngr.configure_server, hostname=server, branch=get_current_git_branch(), git_pull=True
            )
        ...
        while len(self.proc_mngr.active_pids) != len(self.proc_mngr.launched_procs):
            for proc in self.proc_mngr.launched_procs:
                ...
                if proc_info is not None and proc not in self.proc_mngr.active_pids:
                    self.proc_mngr.register_pid(
                        hostname=proc, pid=proc_info["pid"], name=proc_info["name"], ports=proc_info["ports"]
                    )
    ...
    def send_start_cmd(self):
        for server in self.setup["server"]:
            ...
            self.send_cmd(
                ...
                target="server",
                cmd="start",
                cmd_data={"setup": self.setup, "server": server},
                timeout=60,
            )
    ...
    def _start_daq_proc(self, hostname, ignore_orphaned=False):
        ...
        # There is a pid-file
            ...
            if ps_status[hostname]:
                ...
                msg = (
                    "A {0} process is already running on {1}. Only one {0} process at a time can be run on a host. "
                    "Do you want to terminate the {0} process and relaunch a new one?"
                    " Proceeding without terminating the currently running process may lead to faulty behavior".format(
                        proc_kind, pltfrm
                    )
                )
                ...
                reply = QtWidgets.QMessageBox.question(
                    self,
                    "Terminate running {} process and relaunch?".format(proc_kind),
                    msg,
    ...
    def _connect_worker_exception(self, worker):
        worker.signals.exception.connect(
            lambda e, trace: logging.error("{} on sub-thread: {}".format(type(e).__name__, trace))
        )
    ...
    def update_tabs(self):
        ...
        self.monitor_tab = IrradMonitorTab(
            setup=self.setup["server"], parent=self.tabs, plot_path=self.setup["session"]["outfolder"]
        )
    ...
    def handle_data(self, data):
        ...
        # Check whether data is interpreted
            ...
            if data["data"]["status"] == "scan_init":  # Scan is being initialized
                ...
                self.control_tab.tab_widgets[server]["status"].update_status(
                    status="scan",
                    status_values=data["data"],
                    ignore_status=("speed", "accel", "x_start", "x_stop", "y_start", "y_stop"),
                )
                ...
                self.control_tab.tab_widgets[server]["status"].update_status(
                    status="scan",
                    status_values=data["data"],
                    ignore_status=("fluence_hist", "fluence_hist_err", "status"),
                )
            ...
            self.control_tab.tab_widgets[server]["status"].update_status(
                status=data["data"]["axis_domain"], status_values=data["data"], ignore_status=("axis_domain",)
            )
            # Update motorstage positions after every move
            self.control_tab.tab_widgets[server]["motorstage"].update_motorstage_properties(
                motorstage=data["data"]["axis_domain"],
    ...
    def _send_cmd_get_reply(self, hostname, cmd_dict, timeout=None):
        ...
        req_port = (
            self.setup["server"][hostname]["ports"]["cmd"]
            if hostname in self.setup["server"]
            else self.setup["ports"]["cmd"]
        )
        ...
        if _type == "STANDARD":
            if sender == "server":
                if reply == "start":
                    ...
                    for ms, ms_config in reply_data.items():
                        self.control_tab.tab_widgets[hostname]["motorstage"].add_motorstage(
                            motorstage=ms, positions=ms_config["positions"], properties=ms_config["props"]
                        )
                ...
                if reply == "set_ifs":
                    ...
                    self.send_cmd(
                        hostname="localhost", target="interpreter", cmd="record_data", cmd_data=(hostname, True)
                    )
                ...
                if reply == "setup_scan":
                    self.monitor_tab.add_fluence_hist(
                        server=hostname,
                    ...
                    self.control_tab.tab_widgets[hostname]["status"].update_status(
                        status="scan", status_values=reply_data["result"], only_status=("n_rows",)
                    )
            ...
            # Get motorstage responses
                ...
                if reply in ("set_speed", "set_range", "set_accel", "stop"):
                    ...
                    self.control_tab.tab_widgets[hostname]["motorstage"].update_motorstage_properties(
                        motorstage=sender, properties=reply_data["callback"]["result"]
                    )
                elif reply in ["get_speed", "get_range", "get_accel", "get_position"]:
                    prop = reply.split("_")[-1]
                    prop = (
                        {prop: reply_data["result"]}
                        if not isinstance(reply_data["result"], list)
                        else [{prop: r} for r in reply_data["result"]]
                    )
                    self.control_tab.tab_widgets[hostname]["motorstage"].update_motorstage_properties(
                        motorstage=sender, properties=prop
                    )
                elif reply == "get_physical_props":
                    self.control_tab.tab_widgets[hostname]["motorstage"].update_motorstage_properties(
                        motorstage=sender, properties=reply_data["result"]
                    )

                elif reply in ("add_position", "remove_position"):
                    self.control_tab.tab_widgets[hostname]["motorstage"].motorstage_positions_window.validate(
                        motorstage=sender, positions=reply_data["callback"]["result"], validate=reply.split("_")[0]
                    )
    ...
    def _validate_close(self):
        ...
        if len(self._stopped_daq_proc_hostnames) == len(self.proc_mngr.active_pids):
            ...
            for _ in range(10):
                ...
                if not any(
                    self.proc_mngr.active_pids[h][pid]["active"]
                    for h in self.proc_mngr.active_pids
                    for pid in self.proc_mngr.active_pids[h]
                ):
            ...
            # Unfortunately, we could not verify the close, inform user and close
                ...
                msg = (
                    "Shutdown of the converter and server processes could not be validated.\n"
                ...
                msg_box.setStandardButtons(
                    QtWidgets.QMessageBox.Ignore | QtWidgets.QMessageBox.Retry | QtWidgets.QMessageBox.Abort
                )
                reply = msg_box.exec()
    ...
    def _validate_no_scan(self):
        scan_in_progress_servers = [
            s for s in self.setup["server"] if self.control_tab.tab_widgets[s]["scan"].scan_in_progress
        ]

        if scan_in_progress_servers:
            server_names = ",".join(self.setup["server"][s]["name"] for s in scan_in_progress_servers)
            msg = (
                "The following server(s) is currently conducting a scan: {}!\n"
        ...
        # We are initiating the shutdown routine
            ...
            if self._validate_no_scan():
                ...
                for host in self.proc_mngr.active_pids:
                    ...
                    shutdown_worker = QtWorker(
                        func=self._send_cmd_get_reply,
                        hostname=host,
