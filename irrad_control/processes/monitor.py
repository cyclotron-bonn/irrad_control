...
class MonitorGUI(IrradGUI):
    ...
    def _init_input_ui(self):
        ...
        for server in self.setup["server"]:
            ...
            # Update ion type
            for con in [
                lambda ion, s=server: self.setup["server"][s]["daq"].update({"ion": ion}),
            ...
            for con in [
                lambda ene, s=server: self.setup["server"][s]["daq"].update(
                    {"ekin_initial": ene, "ekin": self.ions[self.setup["server"][s]["daq"]["ion"]].ekin_at_dut(ene)}
                ),
                lambda ene, s=server: self.setup["server"][s]["daq"].update(
                    {
                        "lambda": self.ions[self.setup["server"][s]["daq"]["ion"]].calibration(
                            at_energy=ene, as_dict=True
                        )
                    }
                ),
            ]:
        ...
        session_widget.widgets["logging_combo"].currentTextChanged.connect(
            lambda lvl: self.log_widget.change_level(lvl)
        )
        ...
        for con in [
            lambda _, mw=main_widget: mw.setEnabled(False),
    ...
    def _started_daq_proc(self, hostname):
        ...
        if all(s in self._started_daq_proc_hostnames for s in self.setup["server"]):
            ...
            if "localhost" in self._started_daq_proc_hostnames:
                ...
                self.pdiag.setLabelText(
                    "Application launched successfully!\nCompensating raw data offsets of {} server(s)...".format(
