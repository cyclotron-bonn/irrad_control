...
class IrradConverter(DAQProcess):
    ...
    def __init__(self, name=None):
        ...
        self._beam_unstable_std_ratio = (
            5e-2  # Consider beam unstable once it fluctuates by 5% around its mean or the std is 5% of the I_FS
        )
    ...
    def _setup_daq(self):
        ...
        self.interaction_flags = {server: {"write": Event(), "offset": Event()} for server in self.server}
        ...
        self._beam_currents = defaultdict(
            lambda: np.zeros(
                shape=self._shifted_beam_array_length,
                dtype=self.dtypes.generic_dtype(names=["timestamp", "beam", "beam_err"], dtypes=["<f8", "<f4", "<f4"]),
            )
        )
    ...
    def _create_data_entry(self, server, dname, location):
        ...
        self.data_tables[server][dname] = self.output_table.create_table(
            location, description=dtype, name=dname.capitalize()
        )
        # Create arrays
    ...
    def _add_server_data(self, server, server_setup):
        ...
        if "readout" in server_setup:
            ...
            # Fill lookup dicts
            self._lookups[server]["ro_type_idx"] = {
                rt: server_setup["readout"]["types"].index(rt)
                for rt in ro.RO_TYPES
                if rt in server_setup["readout"]["types"]
            }

            self._lookups[server]["sem_foils"] = [
                ch for ch in self._lookups[server]["ro_type_idx"] if "sem" in ch and "sum" not in ch
            ]
            self._lookups[server]["sem_h"] = all(
                x in self._lookups[server]["ro_type_idx"] for x in ("sem_left", "sem_right")
            )
            self._lookups[server]["sem_v"] = all(
                x in self._lookups[server]["ro_type_idx"] for x in ("sem_up", "sem_down")
            )
            self._lookups[server]["offset_ch"] = set(
                [ch for ch in self._lookups[server]["ro_type_idx"] if "ntc" not in ch]
            )
            ...
            for hist_name in ("beam_position", "see_horizontal", "see_vertical", "sey"):
                ...
                self.data_hists[server][hist_name] = {
                    "meta": {"unit": self.hists[actual_hist_type]["unit"], "edges": edges, "centers": centers},
                    "hist": hist,
                }
                ...
                self.output_table.create_array(
                    "/{}/Histogram/{}".format(server_setup["name"], table_name), "edges", edges
                )
                self.output_table.create_array(
                    "/{}/Histogram/{}".format(server_setup["name"], table_name), "centers", centers
                )
                self.output_table.create_array(
                    "/{}/Histogram/{}".format(server_setup["name"], table_name),
                    "unit",
                    np.array([self.hists[actual_hist_type]["unit"]]),
                )
        ...
        if has_ntc_daq_board_ro or "ArduinoNTCReadout" in server_setup["devices"]:
            ...
            if has_ntc_daq_board_ro:
                dtype = self.dtypes.generic_dtype(
                    names=["timestamp", "ntc_channel", "temperature"],
                    dtypes=[
                        "<f8",
                        "<S{}".format(np.max([len(s) for s in server_setup["readout"]["ntc"].values()])),
                        "<f2",
                    ],
                )
                ...
                # Create and store tables
                self.data_tables[server][dname] = self.output_table.create_table(
                    "/{}/Temperature".format(server_setup["name"]), description=dtype, name=node_name
                )
                # Create arrays
                self.data_arrays[server][dname] = np.zeros(shape=1, dtype=dtype)
        ...
        for ms, conf in DEVICES_CONFIG.items():
            if "motorstage" in conf and conf["motorstage"] and ms in server_setup["devices"]:
                ...
                # Create and store tables
                self.data_tables[server][dname] = self.output_table.create_table(
                    "/{}/Motorstage".format(server_setup["name"]), description=dtype, name=node_name
                )
                # Create arrays
                self.data_arrays[server][dname] = np.zeros(shape=1, dtype=dtype)
        ...
        if "RadiationMonitor" in server_setup["devices"]:
            ...
            # Create and store tables
            self.data_tables[server][dname] = self.output_table.create_table(
                "/{}".format(server_setup["name"]), description=dtype, name=node_name
            )
            # Create arrays
            self.data_arrays[server][dname] = np.zeros(shape=1, dtype=dtype)
    ...
    def _setup_daq_parameters(self, server, server_setup):
        ...
        self._daq_params[server]["kappa"] = (
            (np.nan, np.nan)
            if daq_setup["kappa"] is None
            else (daq_setup["kappa"]["nominal"], daq_setup["kappa"]["sigma"])
        )
        self._daq_params[server]["lambda"] = (
            (np.nan, np.nan)
            if daq_setup["lambda"] is None
            else (daq_setup["lambda"]["nominal"], daq_setup["lambda"]["sigma"])
        )

    def _update_ifs_values(self, server):
        for ro_ch, ro_idx in self._lookups[server]["ro_type_idx"].items():
            self._lookups[server]["full_scale_current"][ro_ch] = self._get_full_scale_current(
                server=server, ch_idx=ro_idx, ro_device=self.readout_setup[server]["device"]
            )
    ...
    def _calc_drate(self, server, meta):
        ...
        # Calc data rate
        now = time()
        drate = 1.0 / (now - self._dtimes[server][meta["type"]])
    ...
    def _update_hist_entries(self, server, beam_data):
        hist_data = {"meta": {"timestamp": beam_data["meta"]["timestamp"], "name": server, "type": "hist"}, "data": {}}

        # Update histograms
        # Beam position
        bp_h_idx = analysis.formulas.get_hist_idx(
            val=beam_data["data"]["position"]["h"],
            bin_edges=self.data_hists[server]["beam_position"]["meta"]["edges"][0],
        )
        bp_v_idx = analysis.formulas.get_hist_idx(
            val=beam_data["data"]["position"]["v"],
        ...
        for plane in ("horizontal", "vertical"):
            try:
                ...
                see_idx = analysis.formulas.get_hist_idx(
                    val=see_frac, bin_edges=self.data_hists[server][f"see_{plane}"]["meta"]["edges"]
                )
    ...
    def _extract_scan_currents(self, server):
        ...
        if start_ts >= stop_ts:
            scan_mm = abs(
                self.data_arrays[server]["irrad"]["scan_area_stop_x"][0]
                - self.data_arrays[server]["irrad"]["scan_area_start_x"][0]
            )
            scan_secs = (
                scan_mm / self.data_arrays[server]["scan"]["row_scan_speed"][0]
                + 2
                * self.data_arrays[server]["scan"]["row_scan_speed"][0]
                / self.data_arrays[server]["scan"]["row_scan_accel"][0]
            )
    ...
    def _check_remaining_n_scans(self, server):
        ...
        try:
            ...
            if self.data_arrays[server]["irrad"]["aim_damage"][0] == bytes("neq", encoding="ascii"):
                ...
                remainder_tid -= analysis.formulas.tid_per_scan(
                    primary_fluence=_mean_primary_fluence, stopping_power=self._daq_params[server]["stopping_power"]
                )
                eta_n_scans = remainder_tid / analysis.formulas.tid_per_scan(
                    primary_fluence=self.data_arrays[server]["scan"]["row_primary_fluence"][0],
            ...
            self._check_irrad_event(
                server=server, event_name="IrradiationComplete", trigger_condition=lambda n_s=eta_n_scans: n_s < 1
            )
    ...
    def _interpret_raw_data(self, server, data, meta):
        raw_data = {
            "meta": {"timestamp": meta["timestamp"], "name": server, "type": "raw"},
        ...
        for ch in data:
            ...
            if ch_type in self._lookups[server]["offset_ch"]:
                ...
                raw_data["data"]["current"][ch] = analysis.formulas.v_sig_to_i_sig(
                    v_sig=data[ch],
                ...
                if (
                    "sem_sum" in self._lookups[server]["ro_type_idx"]
                    and self._lookups[server]["ro_type_idx"]["sem_sum"] == ch_idx
                ):
    ...
    def _interpret_beam_data(self, server, data, meta):
        beam_data = {
            "meta": {"timestamp": meta["timestamp"], "name": server, "type": "beam"},
        ...
        if "sem_sum" in self._lookups[server]["ro_type_idx"]:
            ...
            beam_current = analysis.formulas.calibrated_beam_current(
                beam_monitor_sig=sig,
                calibration_factor=ufloat(*self._daq_params[server]["lambda"]),
                full_scale_current=ufloat(sum_ifs, 0.0173 * sum_ifs),
            )

            self.data_arrays[server]["beam"]["beam_current"] = beam_data["data"]["current"]["beam_current"] = (
                beam_current.n
            )
            self.data_arrays[server]["beam"]["beam_current_error"] = beam_data["data"]["current"][
                "beam_current_error"
            ] = beam_current.s

            # Calculate sum SE current
            # dname: see_total
            see_per_surface = analysis.formulas.v_sig_to_i_sig(
                v_sig=sig, full_scale_current=sum_ifs, full_scale_voltage=self._lookups[server]["full_scale_voltage"]
            )
            ...
            self.data_arrays[server]["see"]["see_total"] = beam_data["data"]["see"]["see_total"] = (
                see_per_surface * len(self._lookups[server]["sem_foils"])
            )
            ...
            if not self.irrad_events[server].BeamOff.value.is_valid() and "cup" in self._lookups[server]["ro_type_idx"]:
                ...
                fc_current = analysis.formulas.v_sig_to_i_sig(
                    v_sig=self.data_arrays[server]["raw"][fc_channel][0],
            ...
            logging.warning(
                "Beam current cannot be calculated from calibration due to calibration signal of type 'sem_sum' missing"
            )
        ...
        if "blm" in self._lookups[server]["ro_type_idx"]:
            ...
            blm_current = analysis.formulas.v_sig_to_i_sig(
                v_sig=data[self.readout_setup[server]["channels"][blm_idx]],
            ...
            if blm_current <= self.data_arrays[server]["beam"]["beam_current"][0]:
                try:
                    ...
                    if rel_beam_loss >= self._beam_correction_threshold:
                        ...
                        logging.warning(
                            "Correcting extracted beam current from {:.2E} A to {:.2E} A".format(
                                self.data_arrays[server]["beam"]["beam_current"][0], extracted_current
                            )
                        )

                        self.data_arrays[server]["beam"]["beam_current"] = beam_data["data"]["current"][
                            "beam_current"
                        ] = extracted_current
        ...
        if n_foils not in (2, 4):
            ...
            for sem_ch in self._lookups[server]["sem_foils"]:
                ...
                recon_beam_current += analysis.formulas.calibrated_beam_current(
                    beam_monitor_sig=data[self.readout_setup[server]["channels"][sem_ch_idx]],
                    calibration_factor=self._daq_params[server]["lambda"][0],
                    full_scale_current=sem_ch_ifs,
                )
            recon_beam_current /= n_foils

            self.data_arrays[server]["beam"]["reconstructed_beam_current"] = beam_data["data"]["current"][
                "reconstructed_beam_current"
            ] = recon_beam_current
        ...
        if self._lookups[server]["sem_h"]:
            idx_L, idx_R = (
                self._lookups[server]["ro_type_idx"]["sem_left"],
                self._lookups[server]["ro_type_idx"]["sem_right"],
            )
            sig_L, sig_R = (
                data[self.readout_setup[server]["channels"][idx_L]],
                data[self.readout_setup[server]["channels"][idx_R]],
            )

            # Scale voltage signal to current; signals can have different R/O scales
            sig_L = analysis.formulas.v_sig_to_i_sig(
                v_sig=sig_L,
                full_scale_current=self._lookups[server]["full_scale_current"]["sem_left"],
                full_scale_voltage=self._lookups[server]["full_scale_voltage"],
            )
            sig_R = analysis.formulas.v_sig_to_i_sig(
                v_sig=sig_R,
            ...
            self.data_arrays[server]["see"]["see_horizontal"] = beam_data["data"]["see"]["see_horizontal"] = (
                sig_L + sig_R
            )

            # Horizontal fraction of SEE
            try:
                beam_data["data"]["see"]["frac_h"] = (
                    beam_data["data"]["see"]["see_horizontal"] / beam_data["data"]["see"]["see_total"] * 100
                )
            except ZeroDivisionError:
                pass
        ...
        if self._lookups[server]["sem_v"]:
            idx_U, idx_D = (
                self._lookups[server]["ro_type_idx"]["sem_up"],
                self._lookups[server]["ro_type_idx"]["sem_down"],
            )
            sig_U, sig_D = (
                data[self.readout_setup[server]["channels"][idx_U]],
                data[self.readout_setup[server]["channels"][idx_D]],
            )

            # Scale voltage signal to current; signals can have different R/O scales
            sig_U = analysis.formulas.v_sig_to_i_sig(
                v_sig=sig_U,
                full_scale_current=self._lookups[server]["full_scale_current"]["sem_up"],
                full_scale_voltage=self._lookups[server]["full_scale_voltage"],
            )
            sig_D = analysis.formulas.v_sig_to_i_sig(
                v_sig=sig_D,
            ...
            try:
                beam_data["data"]["see"]["frac_v"] = (
                    beam_data["data"]["see"]["see_vertical"] / beam_data["data"]["see"]["see_total"] * 100
                )
            except ZeroDivisionError:
                pass
    ...
    def _check_irrad_event(self, server, event_name, trigger_condition):
        ...
        if triggered_but_inactive or untriggered_but_active:
            self._store_event_parameters(
                server=server, event=event_name, parameters=self.irrad_events[server].to_dict(event=event_name)
            )
    ...
    def _interpret_scan_data(self, server, data, meta):
        ...
        if data["status"] == "scan_init":
            ...
            row_mean_beam_current_err = (
                np.nanstd(scan_currents["beam"]) ** 2
                + (np.sqrt(np.sum(np.square(scan_currents["beam_err"]))) / len(scan_currents)) ** 2
            ) ** 0.5
            ...
            row_primary_fluence = analysis.formulas.fluence_per_scan(
                ion_current=ufloat(row_mean_beam_current, row_mean_beam_current_err),
            ...
            row_tid = analysis.formulas.tid_per_scan(
                primary_fluence=row_primary_fluence, stopping_power=self._daq_params[server]["stopping_power"]
            )
            ...
            if eta_n_scans < 0:
                ...
                row_scan_time = (
                    self.data_arrays[server]["scan"]["row_stop_timestamp"][0]
                    - self.data_arrays[server]["scan"]["row_start_timestamp"][0]
                )
            ...
            scan_data = {
                "meta": {"timestamp": meta["timestamp"], "name": server, "type": "scan"},
                "data": {
                    "fluence_hist": unumpy.nominal_values(self._row_fluence_hist[server]).tolist(),
            ...
            mean_scan_primary_fluence, mean_scan_primary_fluence_err = self._calc_mean_and_error(
                data=unumpy.uarray(row_primary_fluences_last_scan, row_primary_fluences_last_scan_err)
            )

            # Calculate absolute delivered fluence with this scan
            abs_primary_fluence = ufloat(mean_scan_primary_fluence, mean_scan_primary_fluence_err) + ufloat(
            ...
            abs_tid = analysis.formulas.tid_per_scan(
                primary_fluence=abs_primary_fluence, stopping_power=self._daq_params[server]["stopping_power"]
            )
            ...
            # Log
            logging.info(
                "Scan {}: ({:.2E} +- {:.2E}) {}s / cm^2 and ({:.2E} +- {:.2E}) Mrad".format(
                    data["scan"],
            ...
            scan_data = {
                "meta": {"timestamp": meta["timestamp"], "name": server, "type": "damage"},
                "data": {
                    "scan": data["scan"],
            ...
            mean_result_tid = analysis.formulas.tid_per_scan(
                primary_fluence=mean_result_primary_fluence, stopping_power=self._daq_params[server]["stopping_power"]
            )
            ...
            scan_data = {
                "meta": {"timestamp": meta["timestamp"], "name": server, "type": "result"},
                "data": {
                    "scan": int(self.data_arrays[server]["damage"]["scan"][0]),
    ...
    def _interpret_daq_board_ntc_data(self, server, data, meta):
        ...
        ntc_temp = analysis.formulas.get_ntc_temp(
            ntc_voltage=ntc_voltage, ref_voltage=ro.DAQ_BOARD_CONFIG["common"]["voltages"]["2V5p"]
        )
        ...
        if "blm" in ntc_ch_name.lower():
            self._check_irrad_event(
                server=server, event_name="BLMTempHigh", trigger_condition=lambda t=ntc_temp: t > 100
            )
        elif "dut" in ntc_ch_name.lower():
            self._check_irrad_event(
                server=server, event_name="DUTTempHigh", trigger_condition=lambda t=ntc_temp: t > -10
            )
        else:
            self._check_irrad_event(
                server=server, event_name="GenericTempHigh", trigger_condition=lambda t=ntc_temp: t > 100
            )
        ...
        if len(self._ntc_temps[server]) == len(self.readout_setup[server]["ntc"]):
            ntc_data = {
                "meta": {"timestamp": meta["timestamp"], "name": server, "type": "temp_daq_board"},
    ...
    def _interpret_arduino_temp_data(self, server, data, meta):
        temp_data = {"meta": {"timestamp": meta["timestamp"], "name": server, "type": "temp_arduino"}, "data": {}}
        ...
        for temp in data:
            ...
            if "blm" in temp.lower():
                self._check_irrad_event(
                    server=server, event_name="BLMTempHigh", trigger_condition=lambda t=data[temp]: t > 100
                )
            elif "dut" in temp.lower():
                self._check_irrad_event(
                    server=server, event_name="DUTTempHigh", trigger_condition=lambda t=data[temp]: t > -10
                )
            else:
                self._check_irrad_event(
                    server=server, event_name="GenericTempHigh", trigger_condition=lambda t=data[temp]: t > 100
                )
    ...
    def _interpret_rad_monitor_data(self, server, data, meta):
        rad_data = {"meta": {"timestamp": meta["timestamp"], "name": server, "type": "dose_rate"}, "data": {}}

        self.data_arrays[server]["rad_monitor"]["timestamp"] = meta["timestamp"]

        self._check_irrad_event(
            server=server, event_name="DoseRateHigh", trigger_condition=lambda d=data["dose_rate"]: d > 500
        )  # uSv/h
    ...
    def _store_event_parameters(self, server, event, parameters):
        ...
        self.data_arrays[server]["event"]["parameters"] = ",".join(f"{k}={v}" for k, v in parameters.items()).encode(
            "ascii"
        )[:256]
    ...
    def _start_interpreter(self, setup):
        ...
        self.add_daq_stream(
            daq_stream=[
                self._tcp_addr(port=self.setup["server"][server]["ports"]["data"], ip=server) for server in self.server
            ]
        )
        ...
        if target == "interpreter":
            if cmd == "start":
                ...
                self._send_reply(
                    ...
                    sender=target,
                    _type="STANDARD",
                    data=[server, self.interaction_flags[server]["write"].is_set()],
                )
                ...
                self._update_ifs_values(server=server)
                self._store_event_parameters(
                    server=server, event=cmd, parameters={"group": group, "ifs": ifs, "unit": "nA"}
                )
        ...
        for server, server_setup in self.setup["server"].items():
            ...
            for hist_name in self.data_hists[server]:
                ...
                self.output_table.create_array(
                    "/{}/Histogram/{}".format(server_setup["name"], table_name),
                    "hist",
                    self.data_hists[server][hist_name]["hist"],
                )
