...
class RadiationMonitor(ArduinoFreqCounter):
    ...
    def _ramp(self, direction="up", blocking=True):
        ...
        n_seconds_to_ramp *= 2.0  # Give some more time than the ramping should take
        ...
        if blocking:
            while criteria(self.hv.voltage) and n_seconds_to_ramp > 0:
                sleep(1)
                logging.debug(
                    f"Ramping HV to {self.hv.high_voltage if direction == 'up' else 0} V (Current value: {self.hv.voltage} V)"
                )
                n_seconds_to_ramp -= 1

            if n_seconds_to_ramp < 0:
                logging.warning(
                    f"Ramping {direction} voltage resulted in output voltage of {self.hv.voltage} V, ecpected is {self.hv.high_voltage if direction == 'up' else 0} V"
                )
            else:
