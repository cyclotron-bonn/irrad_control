...
BASE_AXIS_CONFIG = {
    "meta": {"misc": None, "last_updated": None, "filename": None, "configured": False},
    "axis": {
        "inverted": False,  # Axis is inverted
        "speed": {"value": None, "unit": None},
        "range": {"value": None, "unit": None},
        "accel": {"value": None, "unit": None},
        "travel": {"value": 0, "unit": None},
        "position": {"value": None, "unit": None},
        "positions": {},
    },
}
...
def save_base_axis_config(config):
    ...
    try:
        ...
        logging.warning(
            f"Could not update axis configuration file at {config['meta']['filename']}. Maybe it is opened by another process?"
        )
    ...
    def wrapper(instance, value, unit):
        ...
        if not instance.error:
            ...
            if any(p in base_axis_func.__name__.lower() for p in ("move", "stop")):
                ...
                instance.config["axis"][prop].update(
                    {"value": getattr(instance, "get_{}".format(prop))(unit=unit), "unit": unit}
                )
            else:
                raise KeyError(
                    "Property {} not in instances config: {}".format(prop, ", ".join(instance.config["axis"].key()))
                )
...
def base_axis_movement_tracker(axis_movement_func, axis_id, zmq_config, axis_domain=None):
    ...
    def movement_wrapper(axis, value, unit):
        ...
        if zmq_config["axis_pubs"][_axis_key]["thread_id"] != get_ident():
            zmq_config["axis_pubs"][_axis_key]["pub"] = create_pub_from_ctx(
                ctx=zmq_config["ctx"], addr=zmq_config["addr"]
            )
        ...
        _data.update(
            {
                "position": axis.get_position(unit="mm"),
        ...
        _data = {
            ...
            "travel": axis.convert_to_unit(travel, "mm"),
            "position": axis.get_position(unit="mm"),
        }
...
class BaseAxis(object):
    ...
    def _check_unit(self, unit, unit_type):
        ...
        if unit not in self.units[unit_type]:
            logging.warning(
                "Unit of {} must be one of '{}'. Using {}!".format(
                    unit_type, ", ".join(self.units[unit_type]), self.units[unit_type][0]
                )
            )
    ...
    def get_physical_props(self, base_unit="mm"):
        ...
        for prop in physical_properties:
            physical_properties[prop] = getattr(self, f"get_{prop}")(
                unit=self._get_physical_prop_unit(prop=prop, base_unit=base_unit)
            )
    ...
    def add_position(self, name, unit, value=None, date=None):
        ...
        # Position info dict
        new_pos = {
            "value": value if value is not None else self.get_position(unit),
            "unit": unit,
            "date": time.asctime() if date is None else date,
        }

        # We're updating an existing position
        if name in self.config["axis"]["positions"]:
            logging.debug(
                "Updating position {} (Last update: {})".format(name, self.config["axis"]["positions"][name]["date"])
            )
...
class BaseAxisTracker(object):
    ...
    def track_axis(self, axis, axis_id, axis_domain=None):
        ...
        self._zmq_config["axis_pubs"][id(axis)] = {
            "pub": create_pub_from_ctx(ctx=self.ctx, addr=self.addr),
            "thread_id": get_ident(),
        }
        ...
        axis.move_abs = MethodType(
            base_axis_movement_tracker(
                axis_movement_func=axis.move_abs, axis_id=axis_id, zmq_config=self._zmq_config, axis_domain=axis_domain
            ),
            axis,
        )
        axis.move_rel = MethodType(
            base_axis_movement_tracker(
                axis_movement_func=axis.move_rel, axis_id=axis_id, zmq_config=self._zmq_config, axis_domain=axis_domain
            ),
            axis,
        )
