...
class ItemTelnetClient(object):
    ...
    def recv(self):
        reply = self._client.read_until(
            bytes(self.cr_lf_token, encoding="utf-8"), timeout=self._client.timeout
        ).rstrip()
...
class ItemLinearStage(BaseAxis):
    ...
    def _move(self, value, unit, absolute=True):
        ...
        def actual_move(t):
            return self._client.send_cmd(
                cmd="MOVETOMM", data=[self.controller_id, t, int(self.get_speed()), int(self.get_accel())]
            )
