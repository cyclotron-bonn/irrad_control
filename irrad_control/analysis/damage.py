...
def main(data, config=None, summary=None):
    ...
    if config is None:
        ...
        for nfile, data_part, config_part, session_basename in data:
            logging.info(
                f"Generating multipart damage distributions from file {session_basename} (file number {nfile + 1})"
            )

            if len(config_part["server"]) != 1:
                raise ValueError(
                    f"Multipart damage analysis only supports input files containing data from 1 server; found {len(len(config_part['server']))}"
                )
            ...
            if nfile == 0:
                results["primary"], errors["primary"], bin_centers["x"], bin_centers["y"] = (
                    fluence.generate_fluence_map(
                        beam_data=data_part[server]["Beam"],
                ...
                fluence_map_part, fluence_map_part_error, _, _ = fluence.generate_fluence_map(
                    beam_data=data_part[server]["Beam"],
                    scan_data=data_part[server]["Scan"],
                    irrad_data=data_part[server]["Irrad"],
                    bins=bins,
                ...
                errors["primary"] = (errors["primary"] ** 2 + fluence_map_part_error**2) ** 0.5
            ...
            if server_config["daq"]["kappa"] is None:
                ...
                errors["neq"] = (
                    (server_config["daq"]["kappa"]["nominal"] * errors["primary"]) ** 2
                    + (results["primary"] * server_config["daq"]["kappa"]["sigma"]) ** 2
                ) ** 0.5
            ...
            if server_config["daq"]["stopping_power"] is None:
                ...
                results["tid"] = formulas.tid_per_scan(
                    primary_fluence=results["primary"], stopping_power=server_config["daq"]["stopping_power"]
                )
                errors["tid"] = formulas.tid_per_scan(
                    primary_fluence=errors["primary"], stopping_power=server_config["daq"]["stopping_power"]
                )
        ...
        results["primary"], errors["primary"], bin_centers["x"], bin_centers["y"] = fluence.generate_fluence_map(
            beam_data=data[server]["Beam"], scan_data=data[server]["Scan"], irrad_data=irrad_data, bins=bins
        )
        # Generate eqivalent fluence map as well as TID map
        if config["daq"]["kappa"] is None:
            ...
            errors["neq"] = (
                (config["daq"]["kappa"]["nominal"] * errors["primary"]) ** 2
                + (results["primary"] * config["daq"]["kappa"]["sigma"]) ** 2
            ) ** 0.5
        ...
        if config["daq"]["stopping_power"] is None:
            ...
            results["tid"] = formulas.tid_per_scan(
                primary_fluence=results["primary"], stopping_power=config["daq"]["stopping_power"]
            )
            errors["tid"] = formulas.tid_per_scan(
                primary_fluence=errors["primary"], stopping_power=config["daq"]["stopping_power"]
            )
    ...
    for damage, map in results.items():
        dut_map, dut_centers_x, dut_centers_y = fluence.extract_dut_map(
            fluence_map=map,
        ...
        dut_error_map, _, _ = fluence.extract_dut_map(
            fluence_map=errors[damage],
            map_bin_centers_x=bin_centers["x"],
            map_bin_centers_y=bin_centers["y"],
            irrad_data=irrad_data,
        )
        ...
        for damage_map, centers_x, centers_y in [
            (map, bin_centers["x"], bin_centers["y"]),
            (dut_map, dut_centers_x, dut_centers_y),
        ]:
            is_dut = damage_map.shape == dut_map.shape

            fig, _ = plotting.plot_damage_map_3d(
                damage_map=damage_map,
                map_centers_x=centers_x,
                map_centers_y=centers_y,
                contour=not is_dut,
                damage=damage,
                ion_name=ion_name,
                server=server,
            ...
            fig, _ = plotting.plot_damage_error_3d(
                damage_map=damage_map,
                error_map=errors[damage] if not is_dut else dut_error_map,
                map_centers_x=centers_x,
                map_centers_y=centers_y,
                contour=not is_dut,
                damage=damage,
            ...
            fig, _ = plotting.plot_damage_map_2d(
                damage_map=damage_map,
                map_centers_x=centers_x,
                map_centers_y=centers_y,
                damage=damage,
            ...
            fig, _ = plotting.plot_damage_map_contourf(
                damage_map=damage_map,
                map_centers_x=centers_x,
                map_centers_y=centers_y,
                damage=damage,
