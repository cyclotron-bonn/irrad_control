...
def main(data, config, summary=None):
    ...
    fig, _ = plotting.plot_beam_current(timestamps=data[server]["Beam"]["timestamp"], beam_current=beam_current)
    figs.append(fig)
    ...
    plot_data = {
        ...
        "ylabel": "#",
        "label": "Beam current over {}".format(
            duration_str_from_secs(seconds=data[server]["Beam"]["timestamp"][-1] - data[server]["Beam"]["timestamp"][0])
        ),
        "title": "Beam current distribution",
        "fmt": "C0",
    }
    plot_data["label"] += ":\n    ({:.2f}{}{:.2f}) nA".format(nanmean(beam_current), "\u00b1", nanstd(beam_current))
    ...
    fig, _ = plotting.plot_relative_beam_position(
        horizontal_pos=data[server]["Beam"]["horizontal_beam_position"],
