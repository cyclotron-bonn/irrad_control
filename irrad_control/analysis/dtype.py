...
_event_dtype = [
    ...
    ("event", "<S64"),
    ("parameters", "<S256"),
]  # Parameters is a comma-separated string of key=value entries e.g. 'param1=123,param2=ramadankareem,param3=1.1, ...' which is cut-off after 256 entries

# Motorstage data type; contains motorstage positions and parameters
_motorstage_dtype = [
    ("timestamp", "<f8"),  # Timestamp [s]
...
_beam_dtype = [
    ("timestamp", "<f8"),  # Timestamp of current measurement [s]
    ...
    (
        ...
        "<f4",
    ),  # Beam current value reconstructed from signal after ADC instead of analog signal [A]
    ...
    ("vertical_beam_position", "<f4"),
]  # Relative y position of the mean of the beam distribution [%]
...
_scan_dtype = [
    ...
    (
        "row_mean_beam_current_error",
        "<f4",
    ),  # Error of the beam current; quadratic addition of std of beam current and measurement error [A]
    ...
    ("row_scan_accel", "<f4"),
]  # Acceleration with which scan speed is approached / slowed down [mm/s^2]

# Scan configuration data type: contains the initial scan configuration.
_irrad_dtype = [
    ("timestamp", "<f8"),  # Posix-timestamp of init [s]
...
_damage_dtype = [
    ...
    ("scan_tid_error", "<f8"),
]  # Error of total-ionizing dose delivered in this scan [Mrad]
...
class IrradHists:
    ...
    def create_hist(self, hist_name, return_edges=True, return_centers=True):
        ...
        if len(hist.shape) == 1:
            ...
            edges = [
                np.linspace(hist_dict["range"][i][0], hist_dict["range"][i][1], hist_dict["bins"][i] + 1)
