...
GROUPED_ANALYSIS_FLAGS = {"irradiation": ("damage", "scan"), "full": ANALYSIS_FLAGS}
...
def main():
    ...
    # We are doing the same analysis on one/multiple files
        ...
        for nfile, data, config, session_basename in input_files(infiles=parsed["infile"]):
            actual_analysis_out_pdf = (
                session_basename + f"_analysis_{analysis_suffix}.pdf"
                if analysis_out_pdf is None
                else analysis_out_pdf[nfile]
            )
                ...
                for _, content in config["server"].items():
                    ...
                    for a_flag in ANALYSIS_FLAGS:
                        if parsed[a_flag]:
                            ...
                            res = getattr(irrad_analysis, a_flag).main(
                                data=data, config=content, summary=results[server]["summary"]
                            )
