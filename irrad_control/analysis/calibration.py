...
def main(data, config, summary=None):
    ...
    assert "cup" in ch_types or "blm" in ch_types, (
        "Channel(s) of type 'cup'/'blm' required for calibration but not found"
    )
    ...
    for sem_ch in sem_calib_channel:
        for cup_ch in cup_calib_channel:
            ...
            cut_data = apply_rel_data_cuts(
                ...
                cut_max=0.98,
            )  # Cut data larger than 98% of *ref_signal_max*

            if cut_data[cup_ch].shape[0] < 100:
                logging.error(
                    f"Insufficient data after cuts! Skipping calibration for cup-type channel '{cup_ch}' vs. sem-type channel '{sem_ch}'"
                )
                continue

            # Perform calibration between the two channels
            calib_result, stat_result, fit_values, misc_arrays = calibrate_sem_vs_cup(
                data=cut_data,
            ...
            fig, _ = plotting.plot_beam_current(
                timestamps=cut_data["timestamp"][stat_mask], beam_current=current_cup_ch[stat_mask], ch_name=cup_ch
            )

            figs.append(fig)

            # Beam current over time
            fig, _ = plotting.plot_calibration(
                calib_data=current_sem_ch[stat_mask],
            ...
            fig, _ = plotting.plot_generic_fig(
                plot_data={
                    "xdata": lambda_stat_array,
...
def calibrate_sem_vs_cup(data, sem_ch_idx, cup_ch_idx, config, update_ifs_events, return_full=False):
    ...
    ifs_sem_ch = generate_ch_ifs_array(
        data=data, config=config, channel_idx=sem_ch_idx, update_ifs_events=update_ifs_events
    )
    ifs_cup_ch = generate_ch_ifs_array(
        data=data, config=config, channel_idx=cup_ch_idx, update_ifs_events=update_ifs_events
    )

    ref_voltage = _get_ref_voltage(config=config)

    # Calibrate current_sem_ch to current_cup_ch in this case
    current_sem_ch = irrad_formulas.v_sig_to_i_sig(
        data[sem_ch], full_scale_current=ifs_sem_ch, full_scale_voltage=ref_voltage
    )
    current_cup_ch = irrad_formulas.v_sig_to_i_sig(
        data[cup_ch], full_scale_current=ifs_cup_ch, full_scale_voltage=ref_voltage
    )
    ...
    gamma_stat_mask = (gamma_stat_array > (gamma_stat.n - 2 * gamma_stat.s)) & (
        gamma_stat_array < (gamma_stat.n + 2 * gamma_stat.s)
    )
    ...
    logging.debug(
        "Discarding {} ({:.2f} %) entries for calibration fit due to 2 sigma cut".format(
            np.count_nonzero(~gamma_stat_mask), 100 * (np.count_nonzero(~gamma_stat_mask) / gamma_stat_mask.shape[0])
        )
    )

    # Do fit
    popt, perr, red_chi = fit(
        xdata=current_cup_ch[gamma_stat_mask],
    ...
    if not 0.05 <= red_chi <= 5:
        logging.warning(
            f"The calibration fit resulted in a red. Chi^2 of {red_chi:.2E} which indicates a faulty fit or model."
        )

    logging.debug(
        f"Calibration of linear model I_SEE = gamma * I_beam / q_ion -> gamma=({100 * gamma_fit.n:.2E}+-{100 * gamma_fit.s:.2E}) % @ red. Chi^2 {red_chi:.2f}"
    )

    logging.info(
        "Beam monitor calibration for '{}' vs '{}': {} 1/V".format(
            cup_ch, sem_ch, "{}=({:.3f}{}{:.3f})".format("\u03bb", lambda_res.n, "\u00b1", lambda_res.s)
        )
    )

    if return_full:
        return (
            (gamma_res, lambda_res),
            (gamma_stat, lambda_stat),
            (popt, perr, red_chi),
            (current_sem_ch, current_cup_ch, lambda_stat_array, gamma_stat_mask),
        )
    else:
...
def apply_rel_data_cuts(data, ref_sig, ref_sig_max, cut_slope=0.01, cut_min=0.01, cut_max=0.99, return_mask=False):
    ...
    logging.debug(
        "Masking {} ({:.2f} %) entries due to large (< {} % of ref. signal) changes".format(
            np.count_nonzero(~mask_slope), 100 * (np.count_nonzero(~mask_slope) / mask_slope.shape[0]), cut_slope
        )
    )
    ...
    logging.debug(
        "Masking {} ({:.2f} %) entries due to low (> {} % of ref. signal) signal".format(
            np.count_nonzero(~mask_min), 100 * (np.count_nonzero(~mask_min) / mask_min.shape[0]), cut_min
        )
    )
    ...
    logging.debug(
        "Masking {} ({:.2f} %) entries due to high (< {} % of ref. signal) signal".format(
            np.count_nonzero(~mask_max), 100 * (np.count_nonzero(~mask_max) / mask_max.shape[0]), cut_max
        )
    )
    ...
    logging.info(
        "Masking {} ({:.2f} %) entries due to cuts".format(
            np.count_nonzero(~mask), 100 * (np.count_nonzero(~mask) / mask.shape[0])
        )
    )
