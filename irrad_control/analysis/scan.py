...
def generate_scan_resolved_damage_map(scan_data, irrad_data, damage="row_primary_fluence"):
    ...
    logging.info(
        f"Generating row- and scan-resolved {damage} distribution for {n_rows} rows, {n_complete_scans} complete and {n_individual_scans} correction scans..."
    )
...
def generate_scan_overview(scan_data, damage_data, irrad_data):
    ...
    for scan in range(n_complete_scans):
        ...
        overview["row_hist"]["primary_damage"][(scan + 1) * n_rows :] = np.tile(
            offset_future_scans, n_complete_scans - (scan + 1)
        )
...
def main(data, config, summary=None):
    ...
    for dmg in ("row_primary_fluence", "row_tid"):
        resolved_map, n_comp = generate_scan_resolved_damage_map(
            scan_data=data[server]["Scan"], irrad_data=data[server]["Irrad"], damage=dmg
        )

        fig, _ = plotting.plot_scan_damage_resolved(
            resolved_map,
    ...
    scan_overview = generate_scan_overview(
        scan_data=data[server]["Scan"], damage_data=data[server]["Damage"], irrad_data=data[server]["Irrad"]
    )
    ...
    fig, _ = plotting.plot_scan_overview(
        overview=scan_overview, beam_data=data[server]["Beam"], temp_data=temp_data, daq_config=config["daq"]
    )
    figs.append(fig)

    logging.info("Analyse beam properties during scan...")
    # Beam current histogram
    beam_during_scan_mask = create_beam_scan_mask(beam_data=data[server]["Beam"], scan_data=data[server]["Scan"])
    ...
    beam_currents_during_scan[~beam_during_scan_mask] = (
        np.nan
    )  # Mask non-scanning values with np.nan so matplotlib wont connect data
    ...
    if temp_data is not None:
        for temp_set in temp_data.dtype.names:
            if "dut" in temp_set.lower():
                summary["irrad"]["temp"] = np.nanmean(
                    win_from_timestamps(
                        temp_data["timestamp"],
                        temp_data[temp_set],
                        beam_during_scan["timestamp"][0],
                        beam_during_scan["timestamp"][-1],
                    )[1]
                )
                break
    ...
    fig, _ = plotting.plot_beam_current(
        timestamps=data[server]["Beam"]["timestamp"][scan_idx_start:scan_idx_end],
    ...
    plot_data = {
        ...
        "ylabel": "#",
        "label": "Beam current during {} scan".format(
            duration_str_from_secs(seconds=beam_during_scan["timestamp"][-1] - beam_during_scan["timestamp"][0])
        ),
        "title": "Beam current distribution during scan",
        "fmt": "C0",
    }
    plot_data["label"] += ":\n    ({:.2f}{}{:.2f}) nA".format(
        np.nanmean(beam_currents_during_scan[beam_during_scan_mask]),
        "\u00b1",
        np.nanstd(beam_currents_during_scan[beam_during_scan_mask]),
    )
    ...
    fig, _ = plotting.plot_relative_beam_position(
        horizontal_pos=beam_during_scan["horizontal_beam_position"],
    ...
    fig, _ = plotting.plot_fluence_distribution(
        fluence_data=data[server]["Scan"]["row_primary_fluence"],
        ion=config["daq"]["ion"],
