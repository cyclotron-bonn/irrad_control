...
class MotorStageControlWidget(ControlWidget):
    ...
    def _init_widget(self):
        ...
        self.motorstage_positions_window.motorstagePosAdded.connect(
            lambda ms, pos: [
                self.send_cmd(
                    hostname=self.server,
                    target=ms,
                    cmd="add_position",
                    cmd_data={"kwargs": {"name": n, **p}, "callback": {"method": "get_positions"}},
                )
                for n, p in pos.items()
            ]
        )

        self.motorstage_positions_window.motorstagePosRemoved.connect(
            lambda ms, pos: self.send_cmd(
                hostname=self.server,
                target=ms,
                cmd="remove_position",
                cmd_data={"kwargs": {"name": pos}, "callback": {"method": "get_positions"}},
            )
        )
    ...
    def _init_buttons(self):
        ...
        ### Connections ###
        master_btn_stop.clicked.connect(
            lambda _: [
                self.send_cmd(
                    hostname=self.server,
                    target=ms,
                    cmd="stop",
                    cmd_data={"callback": {"method": "get_physical_props", "kwargs": {"base_unit": "mm"}}},
                )
        ...
        for x in [
            lambda _: self.motorstage_positions_window.show(),
    ...
    def _update_ui_elements(self, motorstage):
        if self._get_n_axis(motorstage) == 1:
            ...
            self._ms_widgets[motorstage]["label_pos"].setText(
                f"{self.motorstage_properties[motorstage]['position']:.3f} mm"
            )

        else:
            axis_idx = self._ms_widgets[motorstage]["cbx_axis"].currentIndex()
            self._ms_widgets[motorstage]["spxs_range"][0].setValue(
                self.motorstage_properties[motorstage][axis_idx]["range"][0]
            )
            self._ms_widgets[motorstage]["spxs_range"][1].setValue(
                self.motorstage_properties[motorstage][axis_idx]["range"][1]
            )
            self._ms_widgets[motorstage]["spx_speed"].setValue(
                self.motorstage_properties[motorstage][axis_idx]["speed"]
            )
            self._ms_widgets[motorstage]["spx_abs"].setRange(*self.motorstage_properties[motorstage][axis_idx]["range"])
            self._ms_widgets[motorstage]["spx_abs"].setValue(
                self.motorstage_properties[motorstage][axis_idx]["range"][0]
            )
            self._ms_widgets[motorstage]["label_pos"].setText(
                ",    ".join(
                    f"Axis {i}: {p['position']:.3f} mm" for i, p in self.motorstage_properties[motorstage].items()
                )
            )
    ...
    def add_motorstage(self, motorstage, positions, properties):
        ...
        if motorstage not in self.motorstage_properties:
            ...
            if self._get_n_axis(motorstage) == 1:
                ...
                label_curr_pos = QtWidgets.QLabel(
                    ";\t".join(f"Axis {i}: {p['position']} mm" for i, p in enumerate(properties))
                )
            ...
            spxs_range[0].valueChanged.connect(
                lambda v, s=spxs_range[1]: s.setValue(s.value() if v < s.value() else v + 1.0)
            )
            spxs_range[1].valueChanged.connect(
                lambda v, s=spxs_range[0]: s.setValue(s.value() if v > s.value() else v - 1.0)
            )
            ...
            if n_axis > 1:
                ...
                for con in [
                    ...
                    lambda idx: spxs_range[1].setMaximum(
                        DEVICES_CONFIG[motorstage]["init"]["axis_init"][idx]["travel"] * 1e3
                    ),
                    lambda idx: [
                        s.setValue(self.motorstage_properties[motorstage][idx]["range"][i])
                        for i, s in enumerate(spxs_range)
                    ],
                ...
                spxs_range[1].setMaximum(
                    DEVICES_CONFIG[motorstage]["init"]["axis_init"][cbx_axis.currentIndex()]["travel"] * 1e3
                )
                spx_abs.setMaximum(
                    DEVICES_CONFIG[motorstage]["init"]["axis_init"][cbx_axis.currentIndex()]["travel"] * 1e3
                )
            ...
            btn_stop.clicked.connect(
                lambda _, ms=motorstage: self.send_cmd(
                    hostname=self.server,
                    target=ms,
                    cmd="stop",
                    cmd_data={"callback": {"method": "get_physical_props", "kwargs": {"base_unit": "mm"}}},
            ...
            btn_range.clicked.connect(
                lambda _, ms=motorstage: self.send_cmd(
                    ...
                    cmd="set_range",
                    cmd_data={
                        "kwargs": axis_kwargs({"value": [s.value() for s in spxs_range], "unit": "mm"}),
                        "callback": {"method": "get_physical_props", "kwargs": {"base_unit": "mm"}},
                    },
                )
            )
            # Speed
            btn_speed.clicked.connect(
                lambda _, ms=motorstage: self.send_cmd(
                    ...
                    cmd="set_speed",
                    cmd_data={
                        "kwargs": axis_kwargs({"value": spx_speed.value(), "unit": "mm/s"}),
                        "callback": {"method": "get_physical_props", "kwargs": {"base_unit": "mm"}},
                    },
                )
            )
            ...
            btn_rel.clicked.connect(
                lambda _, ms=motorstage: self._send_movement_cmd(
                    motorstage=ms,
                    cmd="move_rel",
                    cmd_data={"kwargs": axis_kwargs({"value": spx_rel.value(), "unit": "mm"}), "threaded": True},
                )
            )  # Movement in separate thread
            # Abs. movement
            btn_abs.clicked.connect(
                lambda _, ms=motorstage: self._send_movement_cmd(
                    motorstage=ms,
                    cmd="move_abs",
                    cmd_data={"kwargs": axis_kwargs({"value": spx_abs.value(), "unit": "mm"}), "threaded": True},
                )
            )  # Movement in separate thread
            # Abs. movement
            btn_pos.clicked.connect(
                lambda _, ms=motorstage: self._send_movement_cmd(
                    motorstage=ms,
                    cmd="move_pos",
                    cmd_data={"kwargs": {"name": cbx_pos.currentText()}, "threaded": True},
                )
            )  # Movement in separate thread
            ...
            self.motorstage_positions_window.add_motorstage(
                motorstage=motorstage, positions=positions, properties=properties
            )
    ...
    def _send_movement_cmd(self, motorstage, cmd, cmd_data):
        ...
        if motorstage in restricted:
                ...
                if restricting_ms is None:
                    ...
                    if mbox.exec_() == QtWidgets.QMessageBox.Ok:
                        self.send_cmd(
                            ...
                            cmd="move_abs",
                            cmd_data={"kwargs": {"value": 0, "unit": "mm"}, "threaded": True},
                        )
            ...
            # We have control over *motorstage* but not the other one
                ...
                msg += (
                    f" {missing_ms} is currently not controlled by irrad_control and its position can not be checked."
                )
...
class ScanControlWidget(ControlWidget):
    ...
    def launch_scan(self):
        self.send_cmd(hostname=self.server, target="__scan__", cmd="_scan_device", cmd_data={"threaded": True})
    ...
    def _init_ui(self):
        ...
        for btn in damage_buttons.buttons():
            btn.toggled.connect(
                lambda _, bg=damage_buttons, sv=spx_damage_val, se=spx_damage_exp: self._damage_toggled(bg, sv, se)
            )

        spx_damage_val.valueChanged.connect(
            lambda v: self.update_scan_params(aim_value=float(f"{v}e{spx_damage_exp.value()}"))
        )
        spx_damage_exp.valueChanged.connect(
            lambda v: self.update_scan_params(aim_value=float(f"{spx_damage_val.value()}e{v}"))
        )

        # Toggle initially
        rbtn_primary.toggle()

        damage_container.add_widget(
            widget=[label_damage_target, spx_damage_val, spx_damage_exp, label_damage_type]
            + damage_buttons.buttons()
            + []
        )
        ...
        dut_rect_container.setToolTip(
            "Define the DUT area relative to the scan origin. Complete scan area will be calculated according to scan speed and beam fwhm"
        )

        label_start = QtWidgets.QLabel("Start:")
        spx_start_x = QtWidgets.QDoubleSpinBox()
        spx_start_x.setRange(-300.0, 300.0)
        ...
        spx_start_y.setRange(-300.0, 300.0)
        ...
        checkbox_scan_rect.setToolTip(
            "Use DUT rectangle as scan rectangle instead. No modifications will be made w.r.t scan speed or beam fwhm"
        )
        checkbox_scan_rect.stateChanged.connect(
            lambda state: self.update_scan_params(dut_rect_is_scan_area=bool(state))
        )

        dut_rect_container.add_widget(
            widget=[label_start, spx_start_x, spx_start_y, label_end, spx_end_x, spx_end_y, checkbox_scan_rect]
        )
        ...
        for i, irr_ev in enumerate(create_irrad_events()):
            ...
            evt_chbx.stateChanged.connect(
                lambda state, ev=irr_ev.name: self.send_cmd(
                    hostname="localhost",
                    target="interpreter",
                    cmd="toggle_event",
            ...
            if self._after_scan_container is None:
                ...
                def _send_rescan_cmd():
                    ...
                    if self._after_scan_container.widgets["rbtn_row"].isChecked():
                        cmd = "_scan_row"
                        cmd_kwargs = {
                            "row": self._after_scan_container.widgets["spx_row"].value(),
                            "speed": self._after_scan_container.widgets["spx_speed"].value(),
                            "repeat": self._after_scan_container.widgets["spx_repeat"].value(),
                        }
                    else:
                        cmd = "_scan_device"
                        cmd_kwargs = {"speed": self._after_scan_container.widgets["spx_speed"].value(), "repeat": 1}
                ...
                rbtn_row.toggled.connect(
                    lambda state: self._after_scan_container.set_widget_read_only(widget=spx_row, read_only=not state)
                )
                rbtn_row.toggled.connect(
                    lambda state: self._after_scan_container.set_widget_read_only(
                        widget=spx_repeat, read_only=not state
                    )
                )
...
class DAQControlWidget(ControlWidget):
    ...
    def _init_ui(self):
        ...
        btn_offset.clicked.connect(
            lambda _: self.send_cmd(hostname="localhost", target="interpreter", cmd="zero_offset", cmd_data=self.server)
        )
        ...
        self.btn_record.clicked.connect(
            lambda _: self.send_cmd(
                ...
                target="interpreter",
                cmd="record_data",
                cmd_data=(self.server, self.btn_record.text() == "Resume"),
            )
        )
        ...
        for action in [
            # Stop recording data
            lambda _: self.send_cmd(
                hostname="localhost", target="interpreter", cmd="record_data", cmd_data=(self.server, False)
            ),
        ...
        for con in [
            lambda state: self.send_cmd(
                hostname=self.server,
                target="RadiationMonitor",
                cmd="_ramp",
                cmd_data={"kwargs": {"direction": "up" if bool(state) else "down", "blocking": True}, "threaded": True},
            ),
        ...
        for con in [
            lambda _, btn=btn_toggle_rad_mon: self.send_cmd(
                hostname=self.server,
                target="RadiationMonitor",
                cmd="_send_data",
