...
class MotorstagePositionWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Connections
        for x in [
            lambda idx: self.btn_save.setText(f"Save {self.tabs.tabText(idx)} changes"),
    ...
    def _init_buttons(self):
        ...
        self.btn_save.clicked.connect(
            lambda _: self._init_edit(motorstage=self.tabs.tabText(self.tabs.currentIndex()), interactive=True)
        )
        self.btn_save.setEnabled(False)
        self.btn_save.clicked.connect(
            lambda _: self.btn_save.setEnabled(self._check_edit(motorstage=self.tabs.tabText(self.tabs.currentIndex())))
        )
    ...
    def _add_motorstage(self, motorstage, travel_range, unit="mm"):
        if motorstage not in self._ms_widgets:
            ...
            name_edit.textChanged.connect(
                lambda text, b=btn_add: btn_add.setEnabled(
                    text not in self._positions_buffer[motorstage] and text != ""
                )
            )
            ...
            btn_add.clicked.connect(
                lambda _, m=motorstage, n=name_edit, u=unit: self._add_position(
                    motorstage=m,
                    name=n.text(),
            ...
            btn_add.clicked.connect(
                lambda _, m=motorstage: [_s.setValue(_s.minimum()) for _s in self._ms_spnbxs[m]]
            )  # Reset values for next addition
            btn_add.clicked.connect(
                lambda _: self.btn_save.setEnabled(
                    self._check_edit(motorstage=self.tabs.tabText(self.tabs.currentIndex()))
                )
            )  # Check if we can enable

            self._containers[motorstage]["add"] = GridContainer(name="Add position")
            self._containers[motorstage]["add"].add_widget(
                widget=[QtWidgets.QLabel("Name:"), name_edit] + self._ms_spnbxs[motorstage] + [btn_add]
            )
    ...
    def _add_position(self, motorstage, name, coordinates, unit, date=None, saved=False):
        ...
        if name not in self._positions_buffer[motorstage]:
            ...
            if len(coordinates) == 1:
                ...
                self._positions_buffer[motorstage][name] = {
                    "value": coordinates[0],
                    "unit": unit,
                    "date": date if date is not None else time.asctime(),
                    "delete": False,
            ...
            for i in range(len(coordinates)):
                ...
                spx.valueChanged.connect(
                    lambda val, axis=i: _update_pos(val=val, axis=axis, ms=motorstage, psname=name)
                )
                spx.valueChanged.connect(
                    lambda _: self.btn_save.setEnabled(
                        self._check_edit(motorstage=self.tabs.tabText(self.tabs.currentIndex()))
            ...
            _widgets = (
                ...
                + spxs
                + [QtWidgets.QLabel("Last updated: {}".format(date)), label_status, chbx_del]
            )

            chbx_del.stateChanged.connect(
                lambda state: self._positions_buffer[motorstage][name].update({"delete": bool(state)})
            )
            chbx_del.stateChanged.connect(
                lambda _: self.btn_save.setEnabled(
                    self._check_edit(motorstage=self.tabs.tabText(self.tabs.currentIndex()))
            ...
            for w in _widgets[:-1]:
                chbx_del.stateChanged.connect(
                    lambda state, m=motorstage, _w=w: self._containers[m]["add"].set_widget_read_only(
                        _w, read_only=bool(state)
                    )
                )
    ...
    def _init_edit(self, motorstage, interactive=False):
        ...
        if interactive and self._are_you_sure:
            ...
            mbox.setText(
                "Are you sure you want to apply changes? Changes are written to the {} configuration file".format(
                    motorstage
                )
            )
