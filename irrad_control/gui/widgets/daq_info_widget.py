...
class DaqInfoWidget(QtWidgets.QWidget):
    ...
    def _init_ui(self):
        ...
        for server in self.servers:
            ...
            # Add to layout
            info_widget.add_widget(
                widget=[
                    self.data_rate_labels[server],
                    self.beam_current_labels[server],
                    interval_spinbox,
                    digit_spinbox,
                ]
            )
    ...
    def _check_n_tables(self, server):
        ...
        while not enough_tables:
            ...
            while n_cols <= (self.n_channels[server] - total_cols):
                ...
                current_headers = [
                    h + " / " + self.unit[server] for h in self.channels[server][total_cols : total_cols + n_cols]
                ]
    ...
    def update_raw_data(self, data):
        ...
        if refresh_time >= self.refresh_interval[server] or self.refresh_interval[server] == 0:
            ...
            if server in self._beam_current_vals:
                self.beam_current_labels[server].setText(
                    "Beam current: ({:.2f} {} {:.2f}) nA".format(
                        self._beam_current_vals[server][0], "\u00b1", self._beam_current_vals[server][1]
                    )
                )
            ...
            for i in range(table.columnCount()):
                ...
                if ch_data is None:
                    ...
                    table.item(0, i).setText(
                        format(float(table.item(0, i).text()), ".{}f".format(self.n_digits[server]))
                    )
                # Update table entries with new data
                    ...
                    val = (
                        "NaN"
                        if data_header not in ch_data[measure]
                        else format(scale * ch_data[measure][data_header], ".{}f".format(self.n_digits[server]))
                    )
    ...
    def update_beam_current(self, beam_data):
        ...
        self._beam_current_vals[server] = (
            actual_data["current"]["beam_current"] * 1e9,
