...
class SessionSetup(BaseSetupWidget):
    ...
    def _init_setup(self):
        ...
        label_out_file.setToolTip(
            "Name of output file containing raw and interpreted data. Cannot contain whitespaces!"
        )
        edit_out_file = QtWidgets.QLineEdit()
        edit_out_file.setPlaceholderText("irradiation_{}".format("_".join(time.asctime().split())))
        edit_out_file.textEdited.connect(
            lambda t, e=edit_out_file: e.setText(t.replace(" ", "_"))
        )  # Don't allow whitespaces
        ...
        combo_logging.addItems(
            [log_levels[lvl] for lvl in sorted([n_lvl for n_lvl in log_levels if isinstance(n_lvl, int)])]
        )
    ...
    def setup(self):
        ...
        setup["outfile"] = os.path.join(
            self.widgets["folder_edit"].text(),
...
class NetworkSetup(BaseSetupWidget):
    ...
    def _init_setup(self):
        ...
        edit_server.textEdited.connect(
            lambda text: btn_add_server.setEnabled(text != "..." and text not in config["server"]["all"])
        )
        edit_server.textEdited.connect(
            lambda text: btn_add_server.setToolTip(
                "IP already in list of known server IPs"
                if text in config["server"]["all"]
                else "Add IP to list of known servers"
            )
        )
        ...
        self.serverIPsFound.connect(
            lambda ips: self.label_status.setText(
                "{} of {} known servers found.".format(len(ips), len(config["server"]["all"]))
            )
        )
    ...
    def _add_to_known_servers(self, ip):
        ...
        reply = QtWidgets.QMessageBox.question(
            self, "Add server IP", msg, QtWidgets.QMessageBox.Yes, QtWidgets.QMessageBox.No
        )
...
class ServerSelection(BaseSetupWidget):
    ...
    def add_selection(self, selection):
        for i, ip in enumerate(selection):
            ...
            edit.setPlaceholderText(
                default
                if ip not in config["server"]["all"]
                else config["server"]["all"][ip]
                if config["server"]["all"][ip] != "none"
                else default
            )
    ...
    def setup(self):
        return {
            ip: con["edit"].text() or con["edit"].placeholderText()
            for ip, con in self.widgets.items()
            if con["checkbox"].isChecked()
        }
...
class ServerSetupWidget(QtWidgets.QWidget):
    ...
    def _update_readout_widget(self, ip, ro_device, layout):
        if self.setup_widgets[ip]["readout_dev"].device == ro_device:
            ...
            remove_widget(
                widget=self.setup_widgets[ip]["readout_dev"], layout=layout, replace_with=ro_device_setup_updated
            )
    ...
    def _init_setup(self, ip, name=None):
        ...
        ro_device_sel.setupChanged.connect(
            lambda state, _ip=ip, _l=_layout: (
                ...
                None
                if "ADCBoard" not in serv_device_sel.widgets
                else serv_device_sel.widgets["ADCBoard"].setChecked(state != "None"),
                None
                if "ADCBoard" not in serv_device_sel.widgets
                else serv_device_sel.widgets["ADCBoard"].setEnabled(not state != "None"),
                None
                if "IrradDAQBoard" not in serv_device_sel.widgets
                else serv_device_sel.widgets["IrradDAQBoard"].setChecked(state == ro.RO_DEVICES.DAQBoard),
                None
                if "IrradDAQBoard" not in serv_device_sel.widgets
                else serv_device_sel.widgets["IrradDAQBoard"].setEnabled(state != ro.RO_DEVICES.DAQBoard),
            )
        )

        # TODO: make this generic for server devices that have a dedicated setup widget
        serv_device_sel.widgets["ArduinoNTCReadout"].stateChanged.connect(
            lambda state: arduino_temp_setup.setVisible(bool(state))
        )
        ...
        # Store widgets
        self.setup_widgets[ip] = {
            "readout_sel": ro_device_sel,
    ...
    def _validate_setup(self):
        ...
        try:
            ...
            for ip in self.server_ips:
                ...
                if (
                    not any(w.isChecked() for _, w in self.setup_widgets[ip]["device"].widgets.items())
                    and self.setup_widgets[ip]["readout_sel"].setup() == "None"
                ):
                    logging.warning(
                        "No readout / server devices selected for server {}. Please choose devices or remove server.".format(
                            ip
                        )
                    )
                    self.isSetup = False
                ...
                if self.setup_widgets[ip]["device"].widgets["ArduinoNTCReadout"].isChecked() and not any(
                    tcb.isChecked() for tcb in self.setup_widgets[ip]["temp"].widgets["ntc_chbxs"]
                ):
...
class NTCSetup(BaseSetupWidget):
    ...
    def setup(self):
        ...
        ntc_names = [
            e.text() or e.placeholderText() for i, e in enumerate(self.widgets["ntc_edits"]) if i in ntc_sensors
        ]
...
class DAQSetup(BaseSetupWidget):
    ...
    def _init_setup(self):
        ...
        label_kappa = QtWidgets.QLabel("Hardness factor %s:" % "\u03ba")
        ...
        label_prop = QtWidgets.QLabel("Calibration factor %s [1/V]:" % "\u03bb")
        label_prop.setToolTip(
            "Constant translating SEM signal to actual proton beam current via I_Beam = %s * I_FS * SEM_%s"
            % ("\u03a3", "\u03bb")
        )
        ...
        # Connections
        combo_ion.currentTextChanged.connect(
            lambda text: self._setup_ion_selection(ion=text, ckappa=combo_kappa, cprop=combo_prop, senergy=spbx_energy)
        )
        spbx_energy.valueChanged.connect(
            lambda _: self._setup_energy_selection(
                ion=combo_ion.currentText(),
                ckappa=combo_kappa,
                cprop=combo_prop,
                senergy=spbx_energy,
                lstop=label_stopping_power_value,
            )
        )
    ...
    def _setup_energy_selection(self, ion, ckappa, cprop, senergy, lstop):
        ckappa_idx = self.ions[ion].hardness_factor(
            at_energy=self.ions[ion].ekin_at_dut(senergy.value()), return_index=True
        )
    ...
    def setup(self):
        ...
        setup["kappa"] = self.ions[setup["ion"]].hardness_factor(
            as_dict=True, at_index=self.widgets["kappa_combo"].currentIndex()
        )
        setup["lambda"] = self.ions[setup["ion"]].calibration(
            as_dict=True, at_index=self.widgets["lambda_combo"].currentIndex()
        )
...
class ReadoutDeviceSetup(BaseSetupWidget):
    ...
    def _init_setup(self):
        ...
        if self.device != ro.RO_DEVICES.DAQBoard:
            ...
            combo_scale.setCurrentIndex(
                ro.RO_ELECTRONICS_CONFIG["ifs_scales"].index(ro.RO_ELECTRONICS_CONFIG["defaults"]["ifs"])
            )
            ...
            checkbox_jumper.stateChanged.connect(
                lambda state, cbx=combo_group_scale: [
                    (
                        cbx[g].clear(),
                        cbx[g].addItems(
                            ro.DAQ_BOARD_CONFIG["common"]["ifs_labels{}".format("_10" if bool(state) else "")]
                        ),
                    )
                    for g in cbx
                ]
        ...
        for i in range(self.n_channels):
            ...
            if self.device != ro.RO_DEVICES.DAQBoard:
                ...
                _edit.textChanged.connect(
                    lambda text, cbx=_cbx_scale, checkbox=checkbox_scale: cbx.setEnabled(
                        checkbox.isChecked() and bool(text)
                    )
                )
                ...
                combo_scale.currentIndexChanged.connect(
                    lambda idx, cbx=_cbx_scale, checkbox=checkbox_scale: cbx.setCurrentIndex(
                        idx if not checkbox.isChecked() else cbx.currentIndex()
                    )
                )
                ...
                _cbx_group.setCurrentIndex(
                    _cbx_group.findText(ro.RO_DEFAULTS["ch_groups"][i]) if i < len(ro.RO_DEFAULTS["ch_names"]) else 1
                )  # ch12
                _cbx_group.currentIndexChanged.connect(lambda _: self._setup_changed())
                input_widgets["group_combos"].append(_cbx_group)

                # Connections
                _edit.textChanged.connect(lambda text, cbx=_cbx_group: cbx.setEnabled(bool(text)))
            ...
            _cbx_type.setCurrentIndex(
                _cbx_type.findText(ro.RO_DEFAULTS["ch_types"][i])
                if i < len(ro.RO_DEFAULTS["ch_names"])
                else ro.RO_TYPES.index("general_purpose")
            )
    ...
    def _handle_ref_channels(self, item, cbx):
        ...
        if last_idx:
            ...
            for rcbx in self.widgets["ref_combos"]:
                if cbx != rcbx:
                    for i in range(rcbx.count()):
                        if rcbx.itemText(i) == str(last_idx + 1) or (
                            idx is None and rcbx.itemText(i) == str(sender_idx)
                        ):
    ...
    def setup(self):
        ...
        readout["types"] = [
            c.currentText()
            for i, c in enumerate(self.widgets["type_combos"])
            if self.widgets["channel_edits"][i].text()
        ]
        readout["ch_numbers"] = [
            i
            if self.widgets["ref_combos"][i].currentText() == "GND"
            else (i, -1 + int(self.widgets["ref_combos"][i].currentText()))
            for i, w in enumerate(self.widgets["channel_edits"])
            if w.text()
        ]

        if readout["device"] != ro.RO_DEVICES.DAQBoard:
            readout["ro_scales"] = [
                ro.RO_ELECTRONICS_CONFIG["ifs_scales"][ro.RO_ELECTRONICS_CONFIG["ifs_labels"].index(c.currentText())]
                for i, c in enumerate(self.widgets["scale_combos"])
                if self.widgets["channel_edits"][i].text()
            ]
        else:
            ...
            readout["ro_group_scales"] = {
                g: ro.DAQ_BOARD_CONFIG["common"]["ifs_scales"][
            ...
            readout["ch_groups"] = [
                c.currentText()
                for i, c in enumerate(self.widgets["group_combos"])
                if self.widgets["channel_edits"][i].text()
            ]
    ...
    def _is_setup(self):
        ...
        check_2 = (
            True
            if "ntc_setup" not in self.widgets
            else True
            if not self.widgets["ntc_chbx"].isChecked()
            else self.widgets["ntc_setup"].isSetup
        )
        check_3 = (
            True
            if "ntc_setup" not in self.widgets
            ...
            if not self.widgets["ntc_chbx"].isChecked()
                ...
                [True for gcbx in self.widgets["group_combos"] if gcbx.currentText() == "ntc" and gcbx.isEnabled()]
            )
            == 1
        )
        ...
        if not check_3:
            logging.warning(
                "No or multiple *ntc* group channels selected. Reading out NTCs requires exactly one channel to be selected as *ntc* group"
            )
