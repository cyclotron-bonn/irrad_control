...
class IrradMonitorTab(QtWidgets.QWidget):
        ...
        for monitor in self.monitors:
            ...
            if "readout" in self.setup[server]:
                ...
                if monitor == "Raw":
                    ...
                    self.plots[server]["current_plot"] = plots.BeamCurrentPlot(
                        channels=channels, ion=self.setup[server]["daq"]["ion"]
                    )
                    ...
                    self.plots[server]["see_current_plot"] = plots.SEECurrentPlot(
                        channels=see_current_channels, name=f"SEE currents for {ion_name_energy}"
                    )
                    plot_wrappers.append(self._create_plot_wrapper(plot_name="see_current_plot", server=server))

                    self.plots[server]["sey_plot"] = plots.SEYHist(
                        name=f"Secondary-Electron-Yield for {ion_name_energy}", xlabel="SEY"
                    )
                    plot_wrappers.append(self._create_plot_wrapper(plot_name="sey_plot", server=server))

                    if "see_horizontal" in see_current_channels:
                        self.plots[server]["sem_h_plot"] = plots.SEEFracHist(
                            name="SEE Horizontal Fraction", xlabel="Horizontal SEE / Total SEE"
                        )
                        plot_wrappers.append(self._create_plot_wrapper(plot_name="sem_h_plot", server=server))

                    if "see_vertical" in see_current_channels:
                        self.plots[server]["sem_v_plot"] = plots.SEEFracHist(
                            name="SEE Vertical Fraction", xlabel="Vertical SEE / Total SEE"
                        )
            ...
            if has_ntc_daq_board_ro or "ArduinoNTCReadout" in self.setup[server]["devices"]:
                if monitor == "Temp":
                    ...
                    if has_ntc_daq_board_ro:
                        ...
                        self.plots[server]["temp_daq_board_plot"] = plots.TemperatureDataPlot(
                            channels=channels, daq_device="DAQBoard"
                        )
                    ...
                    if "ArduinoNTCReadout" in self.setup[server]["devices"]:
                        ...
                        self.plots[server]["temp_arduino_plot"] = plots.TemperatureDataPlot(
                            channels=channels, daq_device="ArduinoNTCReadout"
                        )
            ...
            if "RadiationMonitor" in self.setup[server]["devices"] and monitor == "DoseRate":
                ...
                self.plots[server]["dose_rate_plot"] = plots.RadMonitorDataPlot(
                    channels=channels, daq_device=daq_device
                )
    ...
    def _create_plot_wrapper(self, plot_name, server):
        ...
        self._plot_wrapper_widgets[server][plot_name] = plots.PlotWrapperWidget(
            plot=self.plots[server][plot_name], plot_path=self.plot_path, file_name=file_name
        )
